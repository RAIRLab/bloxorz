...
class Game:
    ...
    def gridToString(self, grid=None):
        if grid is None:
            grid = self.grid
        lines = []
        for x in grid:
    ...
    def isValidProblem(self):
        containsStart = False
        containsGoal = False
        for x in self.grid:
            for y in x:
                containsStart = True if y == initial_state else containsStart
                containsGoal = True if y == goal_state else containsGoal
        return containsStart and containsGoal
...
if __name__ == "__main__":
    ...
    while goodProblems < num_good_problems:
        ...
        if game.isValidProblem() and game.scoreGrid() >= grading_threshold:
            ...
            print()
            file = open('./levels/rock-' + str(goodProblems) + '.txt', 'w')
            file.write(game.gridToString())
            file.close()
    print("Wrote", goodProblems, "good problems")
