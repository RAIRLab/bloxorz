...
Legend:
  'XX' - walkable tile
  'BB' - start tile (also walkable)
  'GG' - goal tile (also walkable)
  '  ' - empty (no tile)
...
def generate_maze_all_params(n: int, m: int, k: int, seed: Optional[int] = None, semicolon: bool = False) -> str:
    """
    Generate an n x m maze text with exactly k 'XX' tiles plus one 'BB' and one 'GG'.
...
def generate_maze(n) -> str:
    ...
    return generate_maze_all_params(r, c, k, semicolon=False, seed=int(time()*1000))